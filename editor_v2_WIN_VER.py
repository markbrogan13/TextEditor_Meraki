...
def save_to_file():
    ...
    WIP_label = ctk.CTkLabel(pop_up, text="Enter Case Number, then Search for a Directory")
    WIP_label.grid(row=0, column=0, padx=75, pady=25, sticky="ew", columnspan=2)
    ...
    submit_button = ctk.CTkButton(pop_up, text="Choose Directory", fg_color=CONFIRM, hover_color=CONFIRM_HOVER,
                                    command=lambda: search_for_file(pop_up, NOTES_SAVED, case_number.get()))
    submit_button.grid(row=1, column=1, padx=(15,75), pady=(0,25), sticky="ew")
    

def search_for_file(window, notes, case_number_val):
    window.withdraw()
    current_dir = os.getcwd()
    temp_dir = ctk.filedialog.askdirectory(parent=app, initialdir=current_dir, title="Select a Download Location")
    if len(temp_dir) > 0 and case_number_val != '':
        print("you chose: %s" % temp_dir)

        pop_up_1 = ctk.CTkToplevel()
        pop_up_1.configure(fg_color=BACKGROUND)
        pop_up_1.wm_title("Saved")
        pop_up_1.attributes("-topmost", True)

        success_label = ctk.CTkLabel(pop_up_1, text="Saving File", font=app_font_tuple_bold)
        success_label.grid(row=0, column=0, padx=75, pady=25, sticky="ew")
        success_button = ctk.CTkButton(pop_up_1, text="Close", fg_color=CONFIRM, hover_color=CONFIRM_HOVER,
                                      command=pop_up_1.destroy)
        success_button.grid(row=1, column=0, padx=75, pady=25, sticky="ew")

        HOME_DIR = temp_dir
        print(HOME_DIR)
        save_to_file_helper(window, notes, case_number_val, temp_dir)
    else:
        pop_up_1 = ctk.CTkToplevel()
        pop_up_1.configure(fg_color=BACKGROUND)
        pop_up_1.wm_title("Error Saving")
        pop_up_1.attributes("-topmost", True)

        error_label = ctk.CTkLabel(pop_up_1, text="Error Saving File",
                                   font=app_font_tuple_bold)
        error_label.grid(row=0, column=0, padx=75, pady=25, sticky="ew")
        error_button = ctk.CTkButton(pop_up_1, text="Cancel", fg_color=CANCEL, hover_color=CANCEL_HOVER,
                                      command=pop_up_1.destroy, text_color="#111111")
        error_button.grid(row=1, column=0, padx=75, pady=25, sticky="ew")

def save_to_file_helper(window, notes, case_number_val, path=os.getcwd()):
    ...
    print(f'{path}/{filename}.txt')

    with open(f'{path}/{filename}.txt', "w+") as f:
        f.write(notes)
        f.close()
    
    # save_checkbox.configure(variable=ctk.StringVar(value="on"))
    window.destroy()
